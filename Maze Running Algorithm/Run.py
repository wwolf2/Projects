...
def isBound(row, col):
    ...
    if (row == 4 and col != 9):
        return True
    if (row == 7 and col != 5 and col !=1 and col != 9):
        return True
    if (row !=3 and row !=8 and col == 3):
        return True
    if (row !=3 and row !=6 and col == 7):
        return True
    return False
#determine if grid at this row/col is a bound grid (end of map)
def isBound(row, col):
...
def spawnDude():
    while True:
        randCol= 2#random.randrange(gridCount)
        randRow= 8#random.randrange(gridCount)
...
def spawnPortal():
    while True:
        randCol= 2#random.randrange(gridCount)
        randRow= 2#random.randrange(gridCount)
