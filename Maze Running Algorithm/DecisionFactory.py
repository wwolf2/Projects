    ...
    def make_decision(self):  # Calculates priority and makes the decision
        ...
        for i in range(0, 4):  # Save the index of the highest priority tile
            if curr_tile.successPrio[i] == maxvalue:
                temp = i
                break
        curr_tile.successPrio[temp] -= 1  # Decreases the priority of the previous path
        self.update_priority(curr_tile.successDirs[temp])
        self.last_nonStar = curr_tile.successDirs[temp]
        if curr_tile.successDirs[temp] == 'up':
            self.last_nonStar = 'down'
        elif curr_tile.successDirs[temp] == 'down':
            self.last_nonStar = 'up'
        elif curr_tile.successDirs[temp] == 'left':
            self.last_nonStar = 'right'
        elif curr_tile.successDirs[temp] == 'right':
            self.last_nonStar = 'left'
        return curr_tile.successDirs[temp]
